...
% periphery-periphery and periphery-brain subnetworks for insomnia data
%
%% Metadata
...
for i = 1:length(inds)
    ...
    for j = 1:length(ind_data)
        ...
        % extract edffilename with patient id
        patedf = insomdata(ind_data(j)).edffile;
        
        % loop over insomresults struct
        for k = 1:length(insomresults)
            
            % extract name
            pat_id = insomresults(k).name;
            
            % check if same patient id
            if strcmp(patedf(1:end-20), pat_id)
                
                % extract tds matrix
                tdsmat = insomresults(k).result;
                
                % cut tdsmat to only periphery-periphery subnetwork
                tdsmat_pp = tdsmat(1:8,1:8,:);
                
                % cut tdsmat to only periphery-brain subnetwork
                tdsmat_pb = tdsmat(9:end,1:8,:);
                
                [srow, scol, sdim] = size(tdsmat_pb);
                % loop over sleep stages
                for s = 1:4
                    tdssum = 0;
                    numsum = 0;
                    % loop over periphery-periphery matrix in x and y direction
                    for m = 1:length(tdsmat_pp)
                        for n = m+1:length(tdsmat_pp)
                            % sum link strengths for each sleep stage
                            tdssum = tdssum+tdsmat_pp(m, n, s);
                            if tdsmat_pp(m, n, s) > 0.07
                                numsum = numsum+1;
                            end
                        end
                    end
                    % loop over periphery-brain matrix in x and y direction
                    for m = 1:srow
                        for n = 1:scol
                            % sum link strengths for each sleep stage
                            tdssum = tdssum+tdsmat_pb(m, n, s);
                            if tdsmat_pb(m, n, s) > 0.07
                                numsum = numsum+1;
                            end
                        end
                    end
                    ...
                    nmean = tdssum/((((length(tdsmat_pp)^2) - length(tdsmat_pp))/2) + srow*scol);
...
% set figure property
h1 = figure(1)
set(h1, 'units', 'normalized', 'Position', [.3 .3 .3 .6])
...
ylim([0 0.6])
text(1.1,0.55,'c) P-P/P-G')
hold on

h2 = figure(2)
set(h2, 'units', 'normalized', 'Position', [.61 .3 .3 .6])
ylabel('Anzahl Verbindungen')
%legend('Alle', 'alle Frauen', 'alle Männer', '<49', '>59', 'Frauen<49', ...
%    'Frauen>59','Männer<49', 'Männer>59')
ylim([0 600])
text(1.1,550,'f) P-P/P-G')
set(gca, 'Xtick', [1 2 3 4], 'XtickLabel', {'TS' 'LS' 'R' 'W'})
hold on
