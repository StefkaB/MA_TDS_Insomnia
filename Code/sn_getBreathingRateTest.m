...
function [bpi,bpi2,bp,bp2,extrema,extrema1,signal_resampled,dbin]  = sn_getBreathingRateTest(signal,varargin)
...
dbin = find(extrema_dist_bin)

%get extrema with larger maximum than preceding maximum
extrema_max_ltp_bin = extrema(:,2) > [0; extrema(1:end-1,2)];

%get extrema with smaller minimum than following minimum
...
%store the minimal value to follower (shift logical indexes to reach
%follower
...
bp2 = reshape(flipdim(bp,2)',1,2*length(bp));
bpi2 = reshape(flipdim(bpi,2)',1,2*length(bpi));
whos bpi
