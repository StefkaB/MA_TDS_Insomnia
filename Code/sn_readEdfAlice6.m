function [header,signalheader,signalcells ] = sn_readEdfAlice6( subjectfolder,varargin )
...
if ~isempty(edffiles)
    ...
    [header,signalheader,signalcells] = sn_edfScan2matScan('data',fullfile(subjectfolder,edffiles(1).name));
    nfiles = length(edffiles);
    if (nfiles > 1)
        for i =2:nfiles
            %load data
            [header_tmp,signalheader_tmp,signalcells_tmp] = sn_edfScan2matScan('data',fullfile(subjectfolder,edffiles(i).name));
            %add records
            header.num_data_records = header.num_data_records + header_tmp.num_data_records;  
            %cat signals
            for k = 1:header.num_signals
                signalcells{k} = [ signalcells{k}; signalcells_tmp{k}];
            end
        end
    end
end
