function [tdss,nsis] = sn_getSiestaData(siesta,siesta_b,varargin)
...
%
%OUTPUT:
% tds        4-D Vector of (scorr,scorr,4,nrecords), where the last index
%            refers to the number of valid records found in both nights
% nsis       refering nsis-data
%
%MODIFICATION LIST:
%
%------------------------------------------------------------
...
sn_night = arrayfun(@(x) x.night(2), siesta(fn_siesta_idx));
% whos fn_night
% cat the tds_stages
