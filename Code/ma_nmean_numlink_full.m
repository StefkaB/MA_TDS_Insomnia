        ...
        for k = 1:length(insomresults)
            ...
            if strcmp(patedf(1:end-20), pat_id)
                ...
                for s = 1:4
                    ...
                    for m = 1:length(tdsmat)
                        for n = m+1:length(tdsmat)
                            % sum link strengths for each sleep stage
                            tdssum = tdssum+tdsmat(m, n, s);
                            if tdsmat(m, n, s) > 0.07
                                numsum = numsum+1;
                            end
                        end
                    end
                    ...
                    nmean = tdssum/(((length(tdsmat)^2) - length(tdsmat))/2);
